...
class DnsSd {
    ...
    *   - addresses| Array     | Optional | Array of IP addresses to listen on. If not specified,
    *              |           |          | all available network interface addresses will be used.
    * ---------------------------------------------------------------- */
    async discover(params) {
        ...
        if (res['params']['addresses']) {
            this._netif_address_list = res['params']['addresses'];
        } else {
    ...
    _checkDiscoveryParameters(params) {
        ...
        if ('addresses' in params) {
            const v = params['addresses'];
            if (!Array.isArray(v)) {
                return { error: new Error('The `addresses` must be an array.') };
            }
            if (v.length === 0) {
                return { error: new Error('The `addresses` array must not be empty.') };
            }
            for (let i = 0; i < v.length; i++) {
                if (typeof (v[i]) !== 'string' || v[i] === '') {
                    return { error: new Error('The `addresses` must be an array of non-empty strings.') };
                }
                // Basic IPv4 validation
                if (!/^(\d{1,3}\.){3}\d{1,3}$/.test(v[i])) {
                    return { error: new Error(`Invalid IPv4 address: ${v[i]}`) };
                }
            }
            p['addresses'] = v;
        }
    ...
    /* ------------------------------------------------------------------
    * Method: startMonitoring(addresses)
    * - addresses | Array | Optional | Array of IP addresses to listen on.
    *             |       |          | If not specified, all available network
    *             |       |          | interface addresses will be used.
    * ---------------------------------------------------------------- */
    async startMonitoring(addresses) {
        ...
        // Validate addresses parameter if provided
        if (addresses !== undefined) {
            if (!Array.isArray(addresses)) {
                throw new Error('The `addresses` must be an array.');
            }
            if (addresses.length === 0) {
                throw new Error('The `addresses` array must not be empty.');
            }
            for (let i = 0; i < addresses.length; i++) {
                if (typeof (addresses[i]) !== 'string' || addresses[i] === '') {
                    throw new Error('The `addresses` must be an array of non-empty strings.');
                }
                // Basic IPv4 validation
                if (!/^(\d{1,3}\.){3}\d{1,3}$/.test(addresses[i])) {
                    throw new Error(`Invalid IPv4 address: ${addresses[i]}`);
                }
            }
        }
        ...
        // Update the list of network interface IP address
        if (addresses) {
            this._netif_address_list = addresses;
        } else {
            this._netif_address_list = this._getNetifAddressList();
        }
